# X (Twitter) 登入 - 401 錯誤最終解決

> **建立日期**：2025-01-29  
> **錯誤**：`{"code":401,"message":"Missing authorization header"}`  
> **階段**：回調階段（callback）
...
**Supabase 在路由層級要求授權標頭**，但 X 的回調請求來自 X 的伺服器，不會包含 Supabase 的 JWT。

**這是一個 Supabase Edge Functions 的配置問題**，需要在 Supabase Dashboard 中設置 Edge Function 為公開訪問。
...
6. **如果找不到此選項**：
   - 檢查函數的 **「Permissions」** 設定
   - 或查看函數的 **「Details」** 頁面

---

### 方案 2：對比 LINE Edge Function 設定

**如果 LINE 登入正常，對比兩者的設定**：

1. **進入 Edge Functions → line-auth**
2. **查看 `line-auth` 的設定**：
   - JWT 驗證是否關閉？
   - 是否有其他特殊設定？
3. **將 `twitter-auth` 的設定改為與 `line-auth` 相同**

---

### 方案 3：檢查 Supabase 專案的全域設定
...
1. **✅ 對比 LINE Edge Function 設定**（最重要）
   - LINE 登入是否正常工作？
   - 如果 LINE 正常，查看它的設定
   - 將 `twitter-auth` 的設定改為與 `line-auth` 相同

2. **✅ Supabase Dashboard Edge Functions 設定**
...
1. **LINE Edge Function 狀態**：
   - LINE 登入是否正常工作？
   - LINE Edge Function 的設定是什麼？（截圖）

2. **Supabase Dashboard Edge Functions 設定**：
   - `twitter-auth` 函數的設定頁面截圖
   - `line-auth` 函數的設定頁面截圖（如果 LINE 正常）
   - 對比兩者的差異
...
- [X 登入-401錯誤完整解決](./X登入-401錯誤完整解決.md)
