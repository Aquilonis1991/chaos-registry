...
* **廣泛防護範圍**：
  * 留言區防護罩 (`ArenaSection`)：無敵時間鎖定不可被手機調慢時間破解。
...
## 5. 核心互動：留言區
主題內頁的「留言區」是極具特色的 UGC (使用者生成內容) 系統。
* **機制**：用戶需達到一定門檻（如該主題有一定投票參與度）才可發布留言。
* **存在週期 (TTL)**：每則留言具有「生命分鐘數」。只要被按「斥責(Downvote)」時間就會縮短；被按「贊同(Upvote)」就會延長。
* **回收站**：當 TTL 歸零，該留言會被「系統軟回收」，轉為灰色的回收紀錄。
* **鎖定保險**：用戶可花費代幣購買防護罩，保護自己的留言暫時不被時間與斥責所消滅。
...
4. **ArenaMessagesManager (留言區監管)**：強行回收或刪除不當留言。
