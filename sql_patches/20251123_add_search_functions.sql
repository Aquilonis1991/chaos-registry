...
BEGIN
  ...
  SELECT 'topic' AS suggestion_type, tm.title AS suggestion_text, tm.id AS topic_id
  FROM topic_matches tm
  UNION ALL
  SELECT 'tag' AS suggestion_type, tg.display_tag AS suggestion_text, NULL::UUID AS topic_id
  FROM tag_matches tg
