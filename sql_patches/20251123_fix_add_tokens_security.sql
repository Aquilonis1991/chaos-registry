-- ==========================================
-- 修復 add_tokens 安全漏洞
-- 問題：當前 add_tokens 函數沒有任何驗證，用戶可以直接調用無限增加代幣
-- 解決方案：添加驗證，確保只能增加自己的代幣，並且只能通過授權的業務邏輯調用
-- ==========================================

-- 1. 先刪除舊函數，然後創建一個安全的 add_tokens 函數
-- 注意：必須保持原來的參數名稱（user_id, token_amount），否則會報錯
...
BEGIN
  ...
  -- 驗證 4: 代幣數量不能超過合理範圍（防止異常大的數值）
  IF token_amount > 10000 THEN
    RAISE EXCEPTION 'Invalid token amount: Exceeds maximum allowed (10000)';
  END IF;
...
-- 2. 創建一個專門用於觀看廣告的函數，包含業務邏輯驗證
-- 注意：這個函數應該只在 Edge Function 中調用，不應該直接從前端調用
...
-- 4. 驗證函數
SELECT 
  '✅ add_tokens 函數已更新，包含安全驗證' as status,
  proname as function_name,
  pg_get_functiondef(oid) as function_definition
FROM pg_proc
WHERE proname IN ('add_tokens', 'add_tokens_from_ad_watch')
