-- UPDATE V5 RPC: Enable 'Last Login' and 'Created At' from auth.users
-- Now that the function is stable, we can fetch the real login timestamp.
...
BEGIN
    ...
    WITH filtered_users AS (
        SELECT 
            ...
            p.avatar,
            p.tokens,
            p.created_at, -- Keep profile creation time (or use u.created_at if preferred)
            
            -- [CHANGE] Fetch real last sign in from auth.users
...
-- Anon access revoked in previous step, keep it that way.
