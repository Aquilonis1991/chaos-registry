  ...
  try {
    ...
    if (action === "generate") {
      ...
      const trendHint = trendingTopics.length > 0
        ? `\n\n[優先任務]以下是目前 App 內討論度最高的話題，每行格式是「編號 | 話題標題（票數）」：\n${trendingTopics
            .map((t: any, i: number) => `${i + 1} | ${t.title}（目前 ${t.total_votes ?? 0} 票）`)
            .join("\n")}\n請主動想辦法從這幾個話題裡挑一個連結進這篇文案，優先程度高於自己另外發想全新主題。不用直接複述話題的字面內容，可以換個角度連過去，例如：用類似的荒謬提問句型延伸、用同一種「如果...會怎樣」的思考方式呼應、或是用「這讓我想到有人在問類似的事」這種轉場帶過去。只有在真的想不出任何自然的連法、硬套會很牽強時，才維持原本品牌語氣自己發想、不引用任何一則。連結不用你自己寫，系統會自動附加在貼文最後，所以 content 欄位裡絕對不要自己寫任何網址、也不要在文字裡寫「1」「2」這種編號。這則自動附加的純網址不算 CTA 或連結說明句，跟上面「不要寫 CTA」的規則不衝突，不用因此不敢填 topic_id。【規則】只要文案內容有連結到上面任何一則（不管是直接引用還是換角度呼應），就在 "topic_id" 欄位填入該話題前面的編號（純數字，例如 1、2 或 3，不要加任何文字或符號）；真的完全沒有連結任何一則才填 null。`
        : "";
