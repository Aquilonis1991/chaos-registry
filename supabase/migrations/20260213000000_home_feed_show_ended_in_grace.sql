-- 首頁在寬限期內同時顯示進行中與已結束主題（含已產生結語），避免有結語的主題提前消失
-- 條件改為：end_at 在寬限期內 且 status 為 active 或 ended（不再僅限 active）

-- 1. get_hot_topics_with_exposure
...
-- 2. get_latest_topics_with_exposure
...
BEGIN
  ...
  CROSS JOIN LATERAL (
    SELECT
      ...
      CASE t.exposure_level WHEN 'high' THEN 7200 WHEN 'medium' THEN 1800 ELSE 0 END AS exposure_offset,
      LEAST(vote_stats.total_votes, 20) * 60 AS interaction_offset,
      EXTRACT(EPOCH FROM t.created_at)
        + CASE t.exposure_level WHEN 'high' THEN 7200 WHEN 'medium' THEN 1800 ELSE 0 END
        + LEAST(vote_stats.total_votes, 20) * 60 AS new_score
  ) AS score
  WHERE t.is_hidden = false
    AND t.end_at >= now() - v_grace_interval
    AND t.status IN ('active', 'ended')
  ORDER BY score.new_score DESC, t.created_at DESC
  LIMIT p_limit
  OFFSET p_offset;
END;
$$ LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public;
