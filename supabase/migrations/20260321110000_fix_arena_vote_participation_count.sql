-- 修正觀點角鬥場「投票參與度」計算：
-- 原本僅用 votes.amount 加總，但前端 upsert 會覆寫同一 (user_id, topic_id) 列，
-- 多次投票後 amount 仍為單次值，導致誤判「參與不足」。
-- 改為：付費票取 GREATEST( token_transactions 中 cast_vote 的 |amount| 加總 , votes.amount )，
--       免費票仍加 free_votes 筆數。
