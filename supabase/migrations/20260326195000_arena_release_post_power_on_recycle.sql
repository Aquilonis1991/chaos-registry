-- 觀點角鬥場：當留言 TTL 歸零被回收（recycled_at 設定）後，釋放「留言權力」
-- 允許同一 user 在同一 topic 再次發表新留言（仍限制同時間僅能有一則 active 留言）。
--
-- 作法：
-- 1) 將原本的 UNIQUE constraint (topic_id, user_id) 改為「僅 active 列（recycled_at IS NULL）」的 partial unique index
-- 2) post_arena_message RPC 的重複檢查也僅針對 active 列

-- 1) 一人一話題限一則：改為僅限制 active 列
ALTER TABLE public.topic_arena_messages
  DROP CONSTRAINT IF EXISTS topic_arena_messages_one_per_user;

CREATE UNIQUE INDEX IF NOT EXISTS topic_arena_messages_one_per_user_active
  ON public.topic_arena_messages (topic_id, user_id)
  WHERE recycled_at IS NULL;

COMMENT ON INDEX public.topic_arena_messages_one_per_user_active
  IS '每人每話題同時間僅能有一則 active 觀點；回收(recycled_at)後不再佔用名額';

-- 2) post_arena_message：只阻擋 active 留言，回收後可再發
