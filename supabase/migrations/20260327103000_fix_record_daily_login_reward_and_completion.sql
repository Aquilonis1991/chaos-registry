-- Fix daily login claim flow:
-- 1) Use mission_daily_login_reward only
-- 2) Ensure profile/login streak is updated on new daily claim
-- 3) Ensure tokens are granted and mission daily_login is marked completed
-- 4) Return stable reward_tokens for frontend display
...
BEGIN
  ...
  -- Idempotency guard: already claimed today.
  IF v_last_login_date = v_today OR EXISTS (
    SELECT 1
  ...
  -- Resolve reward from the new config key only.
  SELECT (sc.value #>> '{}')
  INTO v_reward_text
  ...
  -- Optional streak milestone reward logic (kept from current behavior).
